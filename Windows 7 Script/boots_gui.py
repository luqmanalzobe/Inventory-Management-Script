...
def normalize(s: str) -> str:
    """UPPERCASE and remove spaces, dashes, dots, underscores, slashes."""
    if not s:
        return ""
    s = s.upper()
    s = re.sub(r'[\s\-._/\\]', '', s)
...
def add_mode(part_input: str, qty: int, box: Optional[int], user: str) -> str:
    ...
    If same PART# but different BOX# -> create NEW row (keep old box).
    If multiple boxes exist and no box -> ask for box.
    """
    wb, header_row, col_map = ensure_workbook()
    ws = wb[SHEET_NAME]
...
def remove_mode(part_input: str, qty: int, user: str, box: Optional[int] = None) -> str:
    """
    If BOX# provided -> remove from that exact PART#+BOX# row.
    If multiple boxes exist and no BOX# -> ask for box.
    If exactly one row -> remove from it.
    If resulting qty == 0 -> delete the row from the sheet.
    """
    wb, header_row, col_map = ensure_workbook()
    ws = wb[SHEET_NAME]
    items = load_inventory(ws, header_row, col_map)
    ...
    if new_qty == 0:
        # Log, then delete the entire row
        write_log(wb, user, "REMOVE/DELETE_ROW", canon, qty_delta=-qty, from_box=target["BOX#"], notes="qty hit 0 -> row deleted")
        ws.delete_rows(target["row"], 1)
        wb.save(EXCEL_PATH)
        return f"Removed {qty} from {canon} (Box: {target['BOX#']}); qty is 0, row deleted."
    else:
        # Keep row; update qty and enforce canonical code
        ws.cell(row=target["row"], column=col_map["PART#"]).value = canon
...
def confirm_action(root: tk.Tk, action: str, part: str, qty: int, box: Optional[int]) -> bool:
    ...
    tk.Label(wrap, text="Confirm " + action.upper(),
             font=("Segoe UI", 14, "bold"), fg=color).pack(anchor="w")
